...
import AlertDialog from '@renderer/components/alert-dialog/alertDialog'
...
export default function RecipientDeleteDialog({
  ...
  const handleDelete = async () => {
    try {
      await window.context.deleteBeneficiary(recipient.beneficiary_id)
    ...
    <AlertDialog open={isOpen} onOpenChange={showActionToggle}>
      <div className="text-center w-72 ">
        ...
        <p className="text-sm text-gray-500">
          Please note, this action can’t be undone. You can’t delete <b>{recipient.full_name}</b> if
          there is a dispersal associated with it.
        </p>
